...
def do1(splitInput):
    locations = prepareWires(splitInput)

    sameLocations = [point1.position for point1 in locations[1] for point2 in locations[2] if point1.position == point2.position]
...
def do2(splitInput):
    ...
    pointsAtSameLocations = [[point1, point2] for point1 in locations[1] for point2 in locations[2] if point1.position == point2.position]
    steps = [point1.steps + point2.steps for point1,point2 in pointsAtSameLocations]

    return min(steps)

def prepareWires(splitInput):
...
def processWires(wire, locations, value):
    ...
    for movement in wire.split(','):
        ...
        deltaMovement = directions[direction]
        
        for _ in range(distance):
            currentPosition = (currentPosition[0] + deltaMovement[0], currentPosition[1] + deltaMovement[1])
            currentPoint = Point(position=currentPosition, steps=step)
            locations[value].append(currentPoint)
            step += 1
