...
def do():      
    ...
    image = [[' ' for x in range(size)] for y in range(size)]

    splitInput = inputSplit(input)
    
    wire1 = splitInput[0].split(',')
    wire2 = splitInput[1].split(',')

    
    
    draw(wire1, image, positionX, positionY, True)
    draw(wire2, image, positionX, positionY, False)
